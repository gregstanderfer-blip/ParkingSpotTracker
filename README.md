...
Every state change is logged and saved as an annotated snapshot, so you have a
record of when vehicles come and go.

## How it works

- It **polls** the camera every `DETECT_INTERVAL_SECONDS`: grabs a single frame
  over RTSP and disconnects, rather than holding the stream open. That keeps WiFi
  bandwidth tiny (a couple of frames every ~15s instead of a 24/7 2K feed) and
  avoids the stream corruption that long, lossy WiFi sessions can cause.
...
- A spot is **occupied** if a detected vehicle's box sits inside the polygon you
  ...
  (hysteresis filters out cars driving past and brief detection dips).
- On a committed change it logs to `events.csv`, saves an annotated image to
  `snapshots/`, and sends an iMessage.
...
- **iMessage alert** on each state change (e.g. *"🅿️ Spot 1 just opened up!"* /
  *"🚗 Spot 1 is now occupied."*). Text-only — see the note below.
- **events.csv** — timestamped log of every occupied/empty change.
- **snapshots/YYYY/MM/DD/** — an annotated image at each change (red = occupied,
  green = empty), sorted into date folders for easy browsing.
...
> text-only. The snapshot image is always saved locally in `snapshots/`.
...
| `DETECT_INTERVAL_SECONDS` | How often it polls (one frame per check). Also sets how little WiFi bandwidth it uses. |
